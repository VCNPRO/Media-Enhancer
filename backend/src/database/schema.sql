...
CREATE TABLE media_files (
  ...
  width INTEGER, -- Width in pixels (for video/image)
  height INTEGER, -- Height in pixels (for video/image)
...
CREATE TABLE projects (
  ...
  settings JSONB, -- Project settings (resolution, fps, etc.)
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE project_media (
  ...
  start_time INTEGER, -- Start time in timeline (seconds)
  end_time INTEGER, -- End time in timeline (seconds)
  transformations JSONB, -- Crop, filters, effects, etc.
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE ai_jobs (
  ...
  job_type VARCHAR(100) NOT NULL, -- 'enhance', 'upscale', 'denoise', 'colorize', etc.
  status VARCHAR(50) DEFAULT 'queued', -- 'queued', 'processing', 'completed', 'failed'
...
CREATE TABLE usage_logs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  action_type VARCHAR(100) NOT NULL, -- 'upload', 'export', 'ai_enhance', etc.
  resource_type VARCHAR(100), -- 'storage', 'export', 'ai_credits'
  amount BIGINT, -- Amount used (bytes for storage, 1 for export, etc.)
  metadata JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
